...
const CookieSlice = createSlice({
    ...
    initialState,
    reducers: {
    addCookieValue: (state, action: PayloadAction< {key: string; CookiesInterface: string } >) => {
            const { key, CookiesInterface } = action.payload;
            state.CookieMap[key] = CookiesInterface;
        },
        updateCookieValue: ( state, action: PayloadAction<{key: string; CookiesInterface: string } >) => {
...
export const { addCookieValue, getCookieValue, updateCookieValue, deleteCookieValue } = CookieSlice.actions;
