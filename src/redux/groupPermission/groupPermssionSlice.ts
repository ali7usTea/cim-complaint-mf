...
export const fetchAllGroupPermssion = createAsyncThunk('GroupPermssion/fetchAllGroupPermssion', async ({ jwtToken, ntLogin }: { jwtToken: string, ntLogin: string }) => {
    ...
    try {
        ...
        Object.keys(permissionDtoMapArray).forEach((key) => {
            groupPermissionMap[key] = permissionDtoMapArray[key] as GroupPermission;
            // Assuming permissionDtoMap[key] is of type GroupPermission
        });

    } catch (error) {
        console.log('Error: GroupPermissionSlice::fetchApiGroupPermssion ', error);
    }

    return groupPermissionMap;
});
export const fetchApiGroupPermssion = createAsyncThunk('GroupPermssion/fetchGroupPermssion', async ({ jwtToken, ntLogin }: { jwtToken: string, ntLogin: string }) => {
    const groupPermissionMap: Record<string, GroupPermission> = {};
    try {
