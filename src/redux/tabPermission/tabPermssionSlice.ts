...
export const fetchAllTabPermssion = createAsyncThunk('TabPermssion/fetchAllTabsPermssion', async ({ jwtToken, ntLogin }: { jwtToken: string; ntLogin: string }) => {
    ...
    try {
        ...
        console.log('Info: tabPermisionSlice::result', result);
        ...
        Object.keys(permissionDtoMapArray).forEach((key) => {
            permissionsMap[key] = permissionDtoMapArray[key] as TabPermission;
        });
    } catch (error) {
        console.log('Error :: tabPermisionSlice::fetchApiTabPermssion: ' + error);
    }

    return permissionsMap;
});

export const fetchApiTabPermssion = createAsyncThunk('TabPermssion/fetchTabPermssion', async ({ jwtToken, ntLogin }: { jwtToken: string; ntLogin: string }) => {
    try {
        ...
        // Directly return the object if it's already in the correct format
        return permissionDtoMapArray as Record<string, TabPermission>;
    } catch (error) {
        console.log('Error :: tabPermisionSlice::fetchApiTabPermssion: ' + error);
...
export const { addTabPermssion, updateTabPermission, deleteTabPermssion, getTabpermission } = TabPermisionslice.actions;
