...
const Agentslice = createSlice({
    ...
    reducers: {
        ...
        addAgents: (state, action: PayloadAction<{ key: string, agent: Agent }>) => {
            const { key, agent } = action.payload;
            state.agents[key] = agent
        },
        updateAgents: (state, action: PayloadAction<{ key: string, updatedAgent: Agent }>) => {
        ...
        getAgent: (state, action: PayloadAction<{ key: string }>) => {
            const { key } = action.payload; const agent = state.agents[key];
            if (agent) { console.log(agent); } else {
...
export const { addAgents, updateAgents, deleteAgents, getAgent } = Agentslice.actions
