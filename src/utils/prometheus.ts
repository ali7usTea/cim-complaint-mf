...
import { CounterLogDetailsType, HistogramLogDetailsType } from '../types/prometheus';
...
const histogram = new Histogram({
    ...
    labelNames: ['event', 'statusCode'],
    buckets: [0.1, 5, 15, 50, 100, 250, 500, 750, 1000, 1500, 2000, 5000, 10000, 15000, 20000],
});
...
export const sendPrometheus = (logDetails: CounterLogDetailsType) => {
    try {
        ...
        clientLogger.error("app.utils.prometheus::sendPrometheus : Error in sendPrometheus - Prometheus counter event error: "+ error);
        console.log('Prometheus counter event error', error);
    }
};

export const sendPrometheusResponseTime = (logDetails: HistogramLogDetailsType) => {
    try {
        const { event, statusCode, responseTime } = logDetails || {};
        histogram.labels(event, statusCode).observe(responseTime);
    } catch (error) {
        clientLogger.error("app.utils.prometheus::sendPrometheusResponseTime : Error in sendPrometheusResponseTime - Prometheus Histogram event error: "+ error);
